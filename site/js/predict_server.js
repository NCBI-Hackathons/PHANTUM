$(function () {
  ...
  $("#file").change(function () {
    ...
    reader.onload = function () {
      ...
      image.onload = function () {
        image_preview_resize(image, '#imgPreview', '#image_data');
        ...
        //   .width(new_width)
        //   .height(new_height);
        // $("#image_data").text(reader.result);
        // console.log($("#image_data").text())
      };
  ...
  $("#predictForm input,#predictForm textarea").jqBootstrapValidation({
    ...
    submitSuccess: function ($form, event) {
      ...
      var image_file_data = $("#image_data").text();
      console.log(image_file_data)
      ...
      $.ajax({
        ...
        success: function (data, textStatus, xhr) {
          ...
          $('#success > .alert-success')
            .append("Data: " + data['message']);
          $('#success > .alert-success')
            .append('</div>');
          // Show original image in corresponding preview area
          var image_orig = new Image();
          image_orig.src = data['original_image'];

          image_orig.onload = function () {
            image_preview_resize(image_orig, '#imgOriginal', '#image_data');
          };
          // Show returned image in corresponding preview area
          var image_ret = new Image();
          image_ret.src = data['return_image'];

          image_ret.onload = function () {
            image_preview_resize(image_ret, '#imgReturned', '#image_data_returned');
          };
          //clear all fields
...
function image_preview_resize(image, imgDiv, srcDiv) {
  console.log(image.width);
  console.log(image.height);
  var max_height = 200;
  var max_width = 200;
